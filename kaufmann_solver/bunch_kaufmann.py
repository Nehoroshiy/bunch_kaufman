...
def regular_kaufman(mtx_origin, alpha=(1. + sqrt(17)) / 8, regularize=False, regularize_coeff=1e-4):
    ...
    while sum < n:
        ...
        nz = min(abs(mtx_view[mtx_view.nonzero()]))
        if nz < regularize_coeff:
            diag = 1.0 / sqrt(abs(mtx_view.mean()))
            print 'MIN NONZERO:'
            print nz, mtx_view.mean()
            print 'diag:'
            print diag
            print 'MTX BEFORE:'
            print mtx
            ...
            print 'PL BEFORE:'
            print PL
            PL[sum:n, sum:n] /= sqrt(diag)
            print 'PL AFTER:'
            print PL
            print 'ASSEMBLED STEP:'
        ...
        mtx_view[0:n_k, n_k:] = 0
        mtx_view[n_k:, 0:n_k] = 0
        PL = dot(PL, -triangular + 2*I(n))
        sum += n_k
        cell_sizes.append(n_k)
    print 'ASSEMBLED:'
    assembled = dot(dot(PL, mtx), np.matrix(PL).getH())
    print assembled
    print 'DIFFERENCE:'
    print frobenius_norm(mtx - assembled)
    print 'MAX ELEM DIFFERENCE:'
    print max_pseudo_norm(mtx - assembled)
    print 'PL:::'
    print PL
    P, L = separate_permutation(PL, dtype=f128)
    if regularize:
        for w, (i, j) in enumerate(zip(*tridiagonal.nonzero())):
            if abs(tridiagonal[i, j]) < regularize_coeff:
...
def bunch_kaufman(mtx_origin, alpha=(1. + sqrt(17)) / 8, regularize=False, regularize_coeff=1e-4):
    ...
    if regularize:
        ...
        print 'Tridiagonal before regularization'
        print T
        print '-'*80
        for w, (i, j) in enumerate(zip(*tridiagonal.nonzero())):
            if abs(tridiagonal[i, j]) < regularize_coeff:
        ...
        for i in xrange(n):
            if tridiagonal[1, i] == 0:
                tridiagonal[1, i] = regularize_coeff
        T = sparse.spdiags(tridiagonal, diags, mtx.shape[0], mtx.shape[0], format='csc').todense()
        print 'Tridiagonal after regularization'
        print T
        print '-'*80
        #for i in xrange(n):
        #    tridiagonal[1, i] *= 1.5
